...
async def translateAudio(source: str = Form(...), target: str = Form(...), username: str = Form(...), audio: UploadFile = File(...)):
    wav_path = upload_path + str(username) + "_" + audio.filename
...
async def splitAudio(source: str = Form(...), target: str = Form(...), username: str = Form(...), audio: UploadFile = File(...)):
    filename = str(username) + "_" + source + "_" + target + '_' + audio.filename 
...
@app.post("/batchTranslateAudio")
async def batchTranslateAudio(
        task_name: str = Form(...),
    ):
    output_folder = Path(upload_path) / task_name
    jsonl_path = output_folder / (task_name + '_out.jsonl')
    output_path = output_folder / (task_name + '_out_translation.jsonl')
    complete_file = output_folder / 'translation_complete'
    ...
    asyncio.create_task(
        run_in_threadpool(
            sync_process_translation_task,
            task_name, str(jsonl_path), str(output_path), str(complete_file), output_folder
        )
    )    

    return {
        "message": "Translation task has started in background",
        "task_name": task_name
    }
...
def sync_process_translation_task(task_name: str, jsonl_path: Path, output_path: Path, complete_file: Path, output_folder: Path):
    try:
        # 读取原始JSONL文件内容
        ...
        for step, batch in tqdm(enumerate(test_dataloader), total=len(test_dataloader)):
            ...
            for idx, (key, audio_path, prompt, output_text, target) in enumerate(zip(
                ...
                if prompt in output_text:
                    ...
                    new_entry["sourceText"] = source_text
                    new_entry["targetText"] = translation_text
                else:
                    source_text, translation_text = "", ""
                    print(output_text)
                    print(f"output_text is invalid, expecting {prompt} in it")
        ...
        print(f"Translation task {task_name} complete. Marker file created: {complete_file}")
    
    except Exception as e:
        print(f"Error processing translation task {task_name}: {str(e)}")
        # 可以在这里创建错误标记文件
        error_file = output_folder / 'translation_error'
        with open(error_file, 'w') as f:
            f.write(str(e))

@app.post("/queryBatchTranslateAudioResult")
async def translateAudioPath(
        task_name: str = Form(...)
    ):
    output_folder = Path(upload_path) / task_name
    ...
    complete = os.path.exists(complete_file)
    jsonl_data = []
...
async def synthesize(
    ...
    output_folder = Path(upload_path) / task_name
    ref_wav_path = output_folder / wav_name
    output_wav_path = output_folder / (Path(wav_name).stem + '_syn.wav')
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            response = await client.post(
                "http://192.168.242.11:9066/synthesize",
                data=form_data,
                timeout=10.0
            )
            ...
            if not output_wav_path.exists():
                raise HTTPException(status_code=500, detail="合成音频文件未生成")
            
            # 方案1：直接返回文件（前端需要处理文件下载）
            # return FileResponse(output_wav_path, media_type="audio/wav")
            
            # 方案2：返回Base64编码的音频数据（更符合你前端的需求）
            async with aiofiles.open(output_wav_path, mode='rb') as file:
        ...
        raise HTTPException(
            ...
            detail=f"调用语音合成服务失败: {e.response.text}"
        )
    except Exception as e:
        raise HTTPException(
            status_code=500,
            detail=f"调用语音合成服务时发生错误: {str(e)}"
        )
